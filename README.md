...
Preview at `http://127.0.0.1:8787`. Pages are static HTML in `public/`. Redirects live in `src/index.js`.

Publish by pushing to GitHub `main`. Do not `wrangler deploy` as the publish path. Do not touch DNS. Squarespace stays the live host until a later cutover.

Canonical host: `https://www.sarth.net`.
