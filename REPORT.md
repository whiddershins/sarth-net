# Report

What the build found that a person has to decide. Regenerated every build.

## Holes

Every `<div class="hole">` on the site and whether `content/holes/` has filled it.
Write the paragraph into the named file, in Sarth's words only, and run `npm run build`.

- filled `content/holes/burlap--how-it-started.md` on https://www.sarth.net/conspiracies/burlap/
- EMPTY  `content/holes/third-wall-studio--how-it-started.md` on https://www.sarth.net/conspiracies/third-wall-studio/
  - wants: Sarth: why a studio, and how the first film with Doron came together. Only the two sentences above are yours so far.
- EMPTY  `content/holes/beautiful-tornado--thoughts.md` on https://www.sarth.net/transmissions/beautiful-tornado/
  - wants: Sarth: how the podcast started with Dom, and whether it is coming back.
- EMPTY  `content/holes/introspections--thoughts.md` on https://www.sarth.net/transmissions/introspections/
  - wants: Sarth: what an Introspections set is, and what you are booking them as now.
- EMPTY  `content/holes/lou-reed-live-at-lollapalooza-2009--thoughts.md` on https://www.sarth.net/transmissions/lou-reed-live-at-lollapalooza-2009/
  - wants: Sarth: the day in Chicago, and what the noise interlude was like from the stage.
- EMPTY  `content/holes/miracles--thoughts.md` on https://www.sarth.net/transmissions/miracles/
  - wants: Sarth: the tracks, where it was recorded, and where a copy lives. Nothing else about this record is on the site yet.
- EMPTY  `content/holes/peggy-sue--thoughts.md` on https://www.sarth.net/transmissions/peggy-sue/
  - wants: Sarth: how the Little Phatty ended up on a Buddy Holly song, and what the session was like.
- EMPTY  `content/holes/singles--thoughts.md` on https://www.sarth.net/transmissions/singles/
  - wants: Sarth: what the singles are as a run, and which one to start with.
- filled `content/holes/solsbury-hill--thoughts.md` on https://www.sarth.net/transmissions/solsbury-hill/
- EMPTY  `content/holes/suspension--thoughts.md` on https://www.sarth.net/transmissions/suspension/
  - wants: Sarth: the mp3.com numbers as you remember them, and what making the first record was like.
- EMPTY  `content/holes/the-creation-of-the-universe--thoughts.md` on https://www.sarth.net/transmissions/the-creation-of-the-universe/
  - wants: Sarth: what you hear on it now, and what the two nights felt like from behind the desk.
- EMPTY  `content/holes/the-family-album--thoughts.md` on https://www.sarth.net/transmissions/the-family-album/
  - wants: Sarth: how the record was made, and which song you would play someone first.
- EMPTY  `content/holes/transmissions-from-the-book-of-sarth--thoughts.md` on https://www.sarth.net/transmissions/transmissions-from-the-book-of-sarth/
  - wants: Sarth: why these four, and what the EP is that the app is not.

## Flat pages

Story pages without the alternating 2/1 and 1/2 bands (Sarth's rule, 23 Sep 2026).
Each carries a `<!-- bands: none. reason -->` comment. Real material only: an image,
a pull quote with its source, an embed, or an excerpt of the record. No filler.

- https://www.sarth.net/conspiracies/awakening-to-blacklist/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspiracies/burlap/ (1 band): Not laid out yet.
- https://www.sarth.net/conspiracies/creation-of-the-universe-ambisonic/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspiracies/gralbum-collective/ (1 band): Not laid out yet.
- https://www.sarth.net/conspiracies/i-just-believe-in-christmas/ (1 band): Not laid out yet.
- https://www.sarth.net/conspiracies/lou-reed-2011-european-tour/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspiracies/lou-reed-song-band/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspiracies/lou-reed-spring-2008-tour/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspiracies/number19/ (1 band): Not laid out yet.
- https://www.sarth.net/conspiracies/reflections-vol-1/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspiracies/third-wall-studio/ (1 band): Not laid out yet.
- https://www.sarth.net/conspiracies/vivid-live/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspiracies/yellow-pony/ (1 band): Not laid out yet.
- https://www.sarth.net/conspirators/laurie-anderson/ (1 band): Not laid out yet.
- https://www.sarth.net/conspirators/leah-coloff/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspirators/lou-reed/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspirators/paul-chuffo/ (0 bands): Not laid out yet.
- https://www.sarth.net/conspirators/tony-diodore/ (0 bands): Not laid out yet.
- https://www.sarth.net/transmissions/miracles/ (0 bands): No cover, track list or store listing found; Sarth to supply.

## Credits

Labels say the name (Sarth's rule, 23 Sep 2026): every story page carries one credit label,
`class="excerpt credit"`, naming Sarth Calhoun and his part in the thing. It is copied into the
twin's `credit:` line so agents get the third-person record first. Pages without one:

None.

## Voice

Sentences say I (Sarth's rule, 23 Sep 2026). Sentences that still name Sarth, outside quotes,
captions, labels and data, with the first one on each page. About and Citations are exempt.

None.
